...
Phase 23. It defines the minimum implementation boundary for a shared catalog
checklist-item-resolution layer over the existing saved-view, navigation,
...
progress, and checklist-item-blocker seams that already exist through the
...
- [docs/architecture/phase-22-handoff.md](./phase-22-handoff.md)
...
Phase 22 closed the gap where progressed assigned reviewed presets could carry
thin per-item blocker state and a single blocker note through package-owned
seams. The next repository-owned gap is still smaller than threaded
collaboration, broader checklist orchestration, broader review workflow
engines, fine-grained RBAC, multi-tenant access, productized discovery, or a
collaborative SaaS catalog: those same blocked presets still do not carry a
stable, package-owned way to record that a blocker has been resolved and
attach a single thin resolution note through the same seams.

The smallest next step is to add a shared checklist-item-resolution layer over
the existing checklist-item-blocker and checklist-item-progress path. This
closes a thin resolution-tracking gap without jumping into a threaded
...
- Primary name: `Cross-Run Audit Catalog Checklist Item Resolutions and Resolution Notes`
- Short label: `Checklist Item Resolutions`

This is the suggested freeze name for Phase 23. If this document is merged, it
becomes the repository-owned formal Phase 23 name.

## Formal Goals

1. Add a shared audit-catalog checklist-item-resolution contract that lets
   existing blocked progressed assigned reviewed presets carry stable per-item
   resolution state and a thin resolution note through package-owned records,
   without snapshotting audit facts or redefining replay and approval source
   of truth.
2. Expose the minimum resolve, list-resolved, inspect-resolution,
   clear-resolution, and apply paths needed for SDK, API, CLI, and web
   surfaces to reopen resolution metadata for blocked progressed assigned
   reviewed presets through the existing seams rather than app-owned storage
   reads or writes.
3. Allow inline and queued execution paths to participate in the same
   checklist-item-resolution rules without introducing a second runtime model,
...
4. Provide the minimum local-development and CI guidance needed to validate
   the shared checklist-item-resolution contract and thin operator-facing
   resolution note surfaces.
5. Document the observability, dashboard, analytics, search, threaded
   ...
   work that remains deferred beyond Phase 23.

## Formal Acceptance Standards

1. A shared audit-catalog checklist-item-resolution contract exists in
   packages and remains a derived layer over existing checklist-item-blocker,
...
2. Operators can add or update thin per-item resolution state and an optional
   resolution note on a visible blocked progressed assigned reviewed preset,
   list entries carrying resolution metadata, inspect resolution metadata,
   clear resolution metadata, and apply visible resolved presets through
   stable references rather than surface-local state.
3. Existing operator surfaces can query or present those resolution paths
...
4. Replay and approval semantics still derive only from persisted runtime and
   approval events.
5. Resolution metadata stores only stable per-item resolution state, minimal
   actor and scope references, an optional thin resolution note, and existing
...
6. At least one inline-originated run and one queued-originated run appear in
   integration coverage for the checklist-item-resolution path.
7. Local-development and CI guidance exists for the Phase 23 path.
8. Phase 23 naming is consistent across changed docs.
9. `pnpm lint` passes.
10. `pnpm typecheck` passes.
11. `pnpm test` passes.
12. `pnpm test:integration` passes.
13. `pnpm build` passes.
14. The phase does not introduce Phase 24 scope, a full observability backend,
...
3. Replacing replay with a checklist-item-resolution model or redefining
...
8. Resolution-driven approval gating, workflow orchestration, or new workflow
   templates.
...
11. Phase 24 or later expansion.
...
- `apps/api`
- `packages/cli`
- `apps/web` only for thin checklist-item-resolution and resolution-note
  presentation and resolved-preset application through existing API seams
- `packages/config` only if minimal resolution defaults need documentation or
  exposure
...
1. Phases 2 through 22 are merged into `main`.
...
3. The database-backed persistence baseline, queued execution path, persisted
   ...
   and checklist-item-blocker baseline are in place and stable.
4. Baseline quality commands are runnable.
5. Replay, audit, observability, and operator seams are stable enough to
   support checklist item resolutions without semantic rework.

At suggested freeze time, these preconditions are satisfied.

## Primary Architecture Risks

1. Accidentally turning checklist item resolutions into a second source of
   truth for replay or approval semantics.
2. Making additive resolution state or resolution notes look required for
   workflow correctness rather than operator-facing audit context.
3. Expanding a resolution phase into a collaborative product, dashboard,
   analytics suite, search product, or permission platform effort.
4. Coupling the shared resolution contract too tightly to web route structure,
   storage layout, or indexing assumptions instead of keeping it as a stable
   read-model boundary.
5. Letting minimal resolution metadata pull in premature collaboration, RBAC,
   org-directory, or multi-tenant product scope.

## Must Be Deferred

The following remain out of scope for Phase 23:

1. Phase 24 or later product and platform expansion.
...
7. Threaded collaborative comments, broader review workflow engines, broader
   ...
   checklist-item-resolution path for this phase.
8. Plugin, marketplace, or broader ecosystem packaging work.
9. Broad event-model redesign beyond the additional derived
   checklist-item-resolution contract.

## Recommended Branch Name Slug

Use:

- `wuxi/phase-23-checklist-item-resolutions`

## Recommended Commit Slicing

Recommended split:

1. docs and ADR alignment for the checklist-item-resolution boundary
2. shared catalog checklist-item-resolution contract
3. persistence and thin API, CLI, SDK, and web resolution wiring
4. integration coverage for inline and queued resolution reads and writes
...
- Split only if the resolution persistence seam must land independently of the
  thin operator-facing resolution-note surfaces.
- Do not use stacked PRs unless a new blocker appears and the reason is
  documented.

## Relationship To Phase 22

Phase 22 established a shared way to attach thin per-item blocker state and a
single blocker note to a progressed assigned reviewed preset. Phase 23 builds
on that foundation by adding a package-owned way to record thin per-item
resolution state and a single resolution note over that same blocked preset
while preserving the same source-of-truth boundary and keeping operator
surfaces thin.

## Why This Is Phase 23

Once progressed assigned reviewed presets can carry per-item blocker state and
a single blocker note, the next repository-owned gap is not a threaded
collaboration product, a broader checklist orchestration engine, a broader
review workflow engine, a fine-grained RBAC system, or a multi-tenant control
plane. It is the lack of a shared, package-owned way to record that a blocker
was resolved and attach a thin resolution note across operator surfaces. A
thin resolution layer closes that smaller gap before broader orchestration,
threaded collaboration, RBAC, SaaS, dashboard, search, or platform concerns
that the repository still defers beyond this phase.

## Start Gate Before Phase 23 Execution

Before Phase 23 implementation begins, verify all of the following:

1. `main` is synced to `origin/main`.
2. There are no open stacked PR dependencies.
3. The working tree is clean.
4. Phase 23 naming is consistent in `AGENTS.md`, `docs/roadmap.md`, and this
   handoff.
