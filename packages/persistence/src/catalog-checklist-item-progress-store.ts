...
import type {
  ...
  CrossRunAuditCatalogChecklistItemProgressStore,
} from "@runroot/replay";
...
export interface InMemoryAuditCatalogChecklistItemProgressStoreOptions {
  readonly progressEntries?: readonly CrossRunAuditCatalogChecklistItemProgress[];
}

export interface FileAuditCatalogChecklistItemProgressStoreOptions {
  readonly filePath: string;
  readonly lockRetryDelayMs?: number;
  readonly lockTimeoutMs?: number;
}

export interface PostgresAuditCatalogChecklistItemProgressStoreOptions
  extends Pick<PostgresRuntimePersistenceOptions, "databaseUrl" | "pool"> {}

export interface SqliteAuditCatalogChecklistItemProgressStoreOptions
  extends Pick<
    SqliteRuntimePersistenceOptions,
    "filePath" | "lockRetryDelayMs" | "lockTimeoutMs"
  > {}

export interface ConfiguredAuditCatalogChecklistItemProgressStoreOptions
...
interface AuditCatalogChecklistItemProgressSnapshot {
  readonly progressEntries: readonly CrossRunAuditCatalogChecklistItemProgress[];
}

let sqliteModulePromise: Promise<SqlJsStatic> | undefined;

export function createConfiguredAuditCatalogChecklistItemProgressStore(
  options: ConfiguredAuditCatalogChecklistItemProgressStoreOptions = {},
): CrossRunAuditCatalogChecklistItemProgressStore {
  const resolved = resolvePersistenceConfig(options);

  switch (resolved.driver) {
    case "file":
      return createFileAuditCatalogChecklistItemProgressStore({
        filePath:
          options.filePath ??
    ...
    case "postgres":
      return createPostgresAuditCatalogChecklistItemProgressStore({
        ...(resolved.databaseUrl ? { databaseUrl: resolved.databaseUrl } : {}),
        ...(options.pool ? { pool: options.pool } : {}),
      });
    case "sqlite":
      return createSqliteAuditCatalogChecklistItemProgressStore({
...
export function createInMemoryAuditCatalogChecklistItemProgressStore(
  options: InMemoryAuditCatalogChecklistItemProgressStoreOptions = {},
): CrossRunAuditCatalogChecklistItemProgressStore {
  const progressEntries = [...(options.progressEntries ?? [])].map((entry) =>
    clone(entry),
  );

  return {
    async deleteCatalogChecklistItemProgress(catalogEntryId) {
      ...
      const [deletedEntry] = progressEntries.splice(existingIndex, 1);

      return deletedEntry ? clone(deletedEntry) : undefined;
    },

    async getCatalogChecklistItemProgress(catalogEntryId) {
      const progress = progressEntries.find(
        (entry) => entry.catalogEntryId === catalogEntryId,
      );

      return progress ? clone(progress) : undefined;
    },

    async listCatalogChecklistItemProgress() {
      return progressEntries
        .slice()
        .sort(compareCatalogChecklistItemProgress)
        .map((entry) => clone(entry));
    },

    async saveCatalogChecklistItemProgress(entry) {
      const existingIndex = progressEntries.findIndex(
      ...
      progressEntries.push(clone(entry));
      progressEntries.sort(compareCatalogChecklistItemProgress);
...
export function createFileAuditCatalogChecklistItemProgressStore(
  options: FileAuditCatalogChecklistItemProgressStoreOptions,
): CrossRunAuditCatalogChecklistItemProgressStore {
    ...
    async deleteCatalogChecklistItemProgress(catalogEntryId) {
      return enqueueAccess(async () =>
        withMutableSnapshot(filePath, options, async (snapshot) => {
          const existingEntry = snapshot.progressEntries.find(
            (entry) => entry.catalogEntryId === catalogEntryId,
          );
          ...
          await writeAuditCatalogChecklistItemProgressSnapshot(filePath, {
            progressEntries: snapshot.progressEntries.filter(
              (entry) => entry.catalogEntryId !== catalogEntryId,
            ),
          });
    ...
    async getCatalogChecklistItemProgress(catalogEntryId) {
      return enqueueAccess(async () =>
        withReadOnlySnapshot(filePath, (snapshot) => {
          const progress = snapshot.progressEntries.find(
            (entry) => entry.catalogEntryId === catalogEntryId,
          );
      ...
      return enqueueAccess(async () =>
        withMutableSnapshot(filePath, options, async (snapshot) => {
          const nextProgressEntries = [
            ...snapshot.progressEntries.filter(
              (candidate) => candidate.catalogEntryId !== entry.catalogEntryId,
            ),
            clone(entry),
          ].sort(compareCatalogChecklistItemProgress);
          ...
          await writeAuditCatalogChecklistItemProgressSnapshot(filePath, {
            progressEntries: nextProgressEntries,
          });
...
export function createPostgresAuditCatalogChecklistItemProgressStore(
  options: PostgresAuditCatalogChecklistItemProgressStoreOptions = {},
): CrossRunAuditCatalogChecklistItemProgressStore {
  const pool = options.pool ?? createDefaultPool(options.databaseUrl);
  let schemaReadyPromise: Promise<void> | undefined;

  return createDatabaseAuditCatalogChecklistItemProgressStore({
    ensureSchema() {
...
export function createSqliteAuditCatalogChecklistItemProgressStore(
  options: SqliteAuditCatalogChecklistItemProgressStoreOptions,
): CrossRunAuditCatalogChecklistItemProgressStore {
  const filePath = resolve(options.filePath);
  let accessQueue = Promise.resolve();
  let schemaReadyPromise: Promise<void> | undefined;
...
function createDatabaseAuditCatalogChecklistItemProgressStore(options: {
  ...
  return {
    async deleteCatalogChecklistItemProgress(catalogEntryId) {
      ...
      return options.withTransaction(async (client) => {
        const existingRows = await client.queryRows<{ data: string }>(
          `SELECT data
           FROM runroot_audit_catalog_checklist_item_progress
        ...
        await client.execute(
          `DELETE FROM runroot_audit_catalog_checklist_item_progress
           WHERE catalog_entry_id = ?`,
          [catalogEntryId],
        );

        return deserializeRow<CrossRunAuditCatalogChecklistItemProgress>(
          existingRows[0].data,
        );
      });
    },
    ...
    async getCatalogChecklistItemProgress(catalogEntryId) {
      ...
      return options.withReadOnlyClient(async (client) => {
        ...
        return rows[0]
          ? deserializeRow<CrossRunAuditCatalogChecklistItemProgress>(
              rows[0].data,
            )
          : undefined;
      });
    ...
    async listCatalogChecklistItemProgress() {
      ...
      return options.withReadOnlyClient(async (client) => {
        ...
        return rows
          .map((row) =>
            deserializeRow<CrossRunAuditCatalogChecklistItemProgress>(row.data),
          )
          .sort(compareCatalogChecklistItemProgress);
      });
    },
    ...
    async saveCatalogChecklistItemProgress(entry) {
      ...
      return options.withTransaction(async (client) => {
        await client.execute(
          `INSERT INTO runroot_audit_catalog_checklist_item_progress (
             catalog_entry_id,
             kind,
             operator_id,
             scope_id,
...
export function resolveAuditCatalogChecklistItemProgressFilePath(
  ...
  return join(
    parsedPath.dir,
    `${parsedPath.name}.audit-catalog-checklist-item-progress.json`,
  );
}
...
async function withReadOnlySnapshot<TValue>(
  filePath: string,
  action: (
    snapshot: AuditCatalogChecklistItemProgressSnapshot,
  ) => TValue | Promise<TValue>,
): Promise<TValue> {
  const snapshot =
    await readAuditCatalogChecklistItemProgressSnapshot(filePath);
...
async function withMutableSnapshot<TValue>(
  filePath: string,
  options: Pick<
    FileAuditCatalogChecklistItemProgressStoreOptions,
  ...
  return withFileLock(filePath, options, async () =>
    action(await readAuditCatalogChecklistItemProgressSnapshot(filePath)),
  );
}

async function readAuditCatalogChecklistItemProgressSnapshot(
  filePath: string,
): Promise<AuditCatalogChecklistItemProgressSnapshot> {
  try {
    const rawSnapshot = await readFile(filePath, "utf8");
    const parsedSnapshot = JSON.parse(
      rawSnapshot,
    ) as AuditCatalogChecklistItemProgressSnapshot;

    return {
      progressEntries: [...(parsedSnapshot.progressEntries ?? [])].map(
        (entry) => clone(entry),
      ),
    };
  } catch (error) {
...
async function writeAuditCatalogChecklistItemProgressSnapshot(
  filePath: string,
  snapshot: AuditCatalogChecklistItemProgressSnapshot,
): Promise<void> {
...
function createDefaultPool(databaseUrl?: string): PostgresPoolLike {
  if (!databaseUrl) {
    throw new Error(
      'Postgres audit catalog checklist item progress requires DATABASE_URL or an explicit "databaseUrl" option.',
    );
  }
...
async function withFileLock<TValue>(
  ...
  options: Pick<
    | FileAuditCatalogChecklistItemProgressStoreOptions
    | SqliteAuditCatalogChecklistItemProgressStoreOptions,
    "lockRetryDelayMs" | "lockTimeoutMs"
  >,
  action: () => Promise<TValue>,
): Promise<TValue> {
  ...
  while (true) {
    try {
      ...
      if (Date.now() - startedAt >= timeoutMs) {
        throw new Error(
          `Timed out waiting for audit catalog checklist item progress lock at "${lockPath}".`,
        );
      }
...
function compareCatalogChecklistItemProgress(
