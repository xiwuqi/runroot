...
export interface InMemoryAuditCatalogChecklistItemSignoffStoreOptions {
  readonly signoffEntries?: readonly CrossRunAuditCatalogChecklistItemSignoff[];
}

export interface FileAuditCatalogChecklistItemSignoffStoreOptions {
  readonly filePath: string;
  readonly lockRetryDelayMs?: number;
  readonly lockTimeoutMs?: number;
}

export interface PostgresAuditCatalogChecklistItemSignoffStoreOptions
  extends Pick<PostgresRuntimePersistenceOptions, "databaseUrl" | "pool"> {}

export interface SqliteAuditCatalogChecklistItemSignoffStoreOptions
  extends Pick<
    SqliteRuntimePersistenceOptions,
    "filePath" | "lockRetryDelayMs" | "lockTimeoutMs"
  > {}

export interface ConfiguredAuditCatalogChecklistItemSignoffStoreOptions
...
interface AuditCatalogChecklistItemSignoffSnapshot {
  readonly signoffEntries: readonly CrossRunAuditCatalogChecklistItemSignoff[];
}

let sqliteModulePromise: Promise<SqlJsStatic> | undefined;

export function createConfiguredAuditCatalogChecklistItemSignoffStore(
  ...
  switch (resolved.driver) {
    case "file":
      return createFileAuditCatalogChecklistItemSignoffStore({
        filePath:
          options.filePath ??
    ...
    case "postgres":
      return createPostgresAuditCatalogChecklistItemSignoffStore({
        ...(resolved.databaseUrl ? { databaseUrl: resolved.databaseUrl } : {}),
        ...(options.pool ? { pool: options.pool } : {}),
      });
    case "sqlite":
      return createSqliteAuditCatalogChecklistItemSignoffStore({
...
export function createInMemoryAuditCatalogChecklistItemSignoffStore(
  options: InMemoryAuditCatalogChecklistItemSignoffStoreOptions = {},
): CrossRunAuditCatalogChecklistItemSignoffStore {
  const signoffEntries = [...(options.signoffEntries ?? [])].map((entry) =>
    clone(entry),
  );

  return {
    async deleteCatalogChecklistItemSignoff(catalogEntryId) {
      const existingIndex = signoffEntries.findIndex(
      ...
      const [deletedEntry] = signoffEntries.splice(existingIndex, 1);

      return deletedEntry ? clone(deletedEntry) : undefined;
    },

    async getCatalogChecklistItemSignoff(catalogEntryId) {
      const signoff = signoffEntries.find(
        (entry) => entry.catalogEntryId === catalogEntryId,
      );

      return signoff ? clone(signoff) : undefined;
    },

    async listCatalogChecklistItemSignoffs() {
      return signoffEntries
        .slice()
        .sort(compareCrossRunAuditCatalogChecklistItemSignoff)
        .map((entry) => clone(entry));
    },

    async saveCatalogChecklistItemSignoff(entry) {
      const existingIndex = signoffEntries.findIndex(
        (candidate) => candidate.catalogEntryId === entry.catalogEntryId,
      );

      if (existingIndex >= 0) {
        signoffEntries.splice(existingIndex, 1);
      }

      signoffEntries.push(clone(entry));
      signoffEntries.sort(compareCrossRunAuditCatalogChecklistItemSignoff);
...
export function createFileAuditCatalogChecklistItemSignoffStore(
  options: FileAuditCatalogChecklistItemSignoffStoreOptions,
): CrossRunAuditCatalogChecklistItemSignoffStore {
  ...
  return {
    async deleteCatalogChecklistItemSignoff(catalogEntryId) {
      return enqueueAccess(async () =>
        withMutableSnapshot(filePath, options, async (snapshot) => {
          const existingEntry = snapshot.signoffEntries.find(
            (entry) => entry.catalogEntryId === catalogEntryId,
          );
          ...
          await writeAuditCatalogChecklistItemSignoffSnapshot(filePath, {
            signoffEntries: snapshot.signoffEntries.filter(
              (entry) => entry.catalogEntryId !== catalogEntryId,
            ),
          });
    ...
    async getCatalogChecklistItemSignoff(catalogEntryId) {
      return enqueueAccess(async () =>
        withReadOnlySnapshot(filePath, (snapshot) => {
          const signoff = snapshot.signoffEntries.find(
            (entry) => entry.catalogEntryId === catalogEntryId,
          );
    ...
    async saveCatalogChecklistItemSignoff(entry) {
      return enqueueAccess(async () =>
        withMutableSnapshot(filePath, options, async (snapshot) => {
          const nextSignoffEntries = [
            ...snapshot.signoffEntries.filter(
              (candidate) => candidate.catalogEntryId !== entry.catalogEntryId,
            ),
            clone(entry),
          ].sort(compareCrossRunAuditCatalogChecklistItemSignoff);
          ...
          await writeAuditCatalogChecklistItemSignoffSnapshot(filePath, {
            signoffEntries: nextSignoffEntries,
          });
...
export function createPostgresAuditCatalogChecklistItemSignoffStore(
  options: PostgresAuditCatalogChecklistItemSignoffStoreOptions = {},
): CrossRunAuditCatalogChecklistItemSignoffStore {
  const pool = options.pool ?? createDefaultPool(options.databaseUrl);
  let schemaReadyPromise: Promise<void> | undefined;

  return createDatabaseAuditCatalogChecklistItemSignoffStore({
    ensureSchema() {
...
export function createSqliteAuditCatalogChecklistItemSignoffStore(
  options: SqliteAuditCatalogChecklistItemSignoffStoreOptions,
): CrossRunAuditCatalogChecklistItemSignoffStore {
  const filePath = resolve(options.filePath);
  let accessQueue = Promise.resolve();
  let schemaReadyPromise: Promise<void> | undefined;
...
function createDatabaseAuditCatalogChecklistItemSignoffStore(options: {
  ...
  return {
    async deleteCatalogChecklistItemSignoff(catalogEntryId) {
      ...
      return options.withTransaction(async (client) => {
        const existingRows = await client.queryRows<{ data: string }>(
          `SELECT data
           FROM runroot_audit_catalog_checklist_item_signoff
        ...
        await client.execute(
          `DELETE FROM runroot_audit_catalog_checklist_item_signoff
           WHERE catalog_entry_id = ?`,
          [catalogEntryId],
        );

        return deserializeRow<CrossRunAuditCatalogChecklistItemSignoff>(
          existingRows[0].data,
        );
      });
    },
    ...
    async getCatalogChecklistItemSignoff(catalogEntryId) {
      ...
      return options.withReadOnlyClient(async (client) => {
        ...
        return rows[0]
          ? deserializeRow<CrossRunAuditCatalogChecklistItemSignoff>(
              rows[0].data,
            )
          : undefined;
      });
    ...
    async listCatalogChecklistItemSignoffs() {
      ...
      return options.withReadOnlyClient(async (client) => {
        ...
        return rows
          .map((row) =>
            deserializeRow<CrossRunAuditCatalogChecklistItemSignoff>(row.data),
          )
          .sort(compareCrossRunAuditCatalogChecklistItemSignoff);
      });
    },
    ...
    async saveCatalogChecklistItemSignoff(entry) {
      ...
      return options.withTransaction(async (client) => {
        await client.execute(
          `INSERT INTO runroot_audit_catalog_checklist_item_signoff (
             catalog_entry_id,
             kind,
             operator_id,
             scope_id,
           ...
           ON CONFLICT (catalog_entry_id) DO UPDATE SET
             ...
             signoff_note = excluded.signoff_note,
             signoff_items = excluded.signoff_items,
...
export function resolveAuditCatalogChecklistItemSignoffFilePath(
  ...
  return join(
    parsedPath.dir,
    `${parsedPath.name}.audit-catalog-checklist-item-signoff.json`,
  );
}
...
async function withReadOnlySnapshot<TValue>(
  filePath: string,
  action: (
    snapshot: AuditCatalogChecklistItemSignoffSnapshot,
  ) => TValue | Promise<TValue>,
): Promise<TValue> {
  const snapshot = await readAuditCatalogChecklistItemSignoffSnapshot(filePath);
...
async function withMutableSnapshot<TValue>(
  filePath: string,
  options: Pick<
    FileAuditCatalogChecklistItemSignoffStoreOptions,
  ...
  return withFileLock(filePath, options, async () =>
    action(await readAuditCatalogChecklistItemSignoffSnapshot(filePath)),
  );
}

async function readAuditCatalogChecklistItemSignoffSnapshot(
  filePath: string,
): Promise<AuditCatalogChecklistItemSignoffSnapshot> {
  try {
    const rawSnapshot = await readFile(filePath, "utf8");
    const parsedSnapshot = JSON.parse(
      rawSnapshot,
    ) as AuditCatalogChecklistItemSignoffSnapshot;

    return {
      signoffEntries: [...(parsedSnapshot.signoffEntries ?? [])].map((entry) =>
        clone(entry),
      ),
    };
  } catch (error) {
...
async function writeAuditCatalogChecklistItemSignoffSnapshot(
  filePath: string,
  snapshot: AuditCatalogChecklistItemSignoffSnapshot,
): Promise<void> {
...
function createDefaultPool(databaseUrl?: string): PostgresPoolLike {
  if (!databaseUrl) {
    throw new Error(
      'Postgres audit catalog checklist item signoff requires DATABASE_URL or an explicit "databaseUrl" option.',
    );
  }
...
async function withFileLock<TValue>(
  ...
  options: Pick<
    | FileAuditCatalogChecklistItemSignoffStoreOptions
    | SqliteAuditCatalogChecklistItemSignoffStoreOptions,
    "lockRetryDelayMs" | "lockTimeoutMs"
  >,
  action: () => Promise<TValue>,
): Promise<TValue> {
  ...
  while (true) {
    try {
      ...
      if (Date.now() - startedAt >= timeoutMs) {
        throw new Error(
          `Timed out waiting for audit catalog checklist item signoff lock at "${lockPath}".`,
        );
      }
