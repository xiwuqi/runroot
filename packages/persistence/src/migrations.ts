...
export const runtimePersistenceMigrations = [
  ...
  {
    id: "0018_audit_catalog_checklist_item_exception",
    postgres: [
      `CREATE TABLE IF NOT EXISTS runroot_audit_catalog_checklist_item_exception (
        catalog_entry_id TEXT PRIMARY KEY,
        kind TEXT NOT NULL,
        operator_id TEXT NOT NULL,
        scope_id TEXT NOT NULL,
        exception_note TEXT,
        exception_items TEXT NOT NULL,
        created_at TEXT NOT NULL,
        updated_at TEXT NOT NULL,
        data TEXT NOT NULL
      )`,
      `CREATE INDEX IF NOT EXISTS idx_runroot_audit_catalog_checklist_item_exception_scope
        ON runroot_audit_catalog_checklist_item_exception (scope_id, updated_at DESC, catalog_entry_id ASC)`,
      `CREATE INDEX IF NOT EXISTS idx_runroot_audit_catalog_checklist_item_exception_operator
        ON runroot_audit_catalog_checklist_item_exception (operator_id, updated_at DESC, catalog_entry_id ASC)`,
    ],
    sqlite: [
      `CREATE TABLE IF NOT EXISTS runroot_audit_catalog_checklist_item_exception (
        catalog_entry_id TEXT PRIMARY KEY,
