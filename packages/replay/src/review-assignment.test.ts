...
describe("@runroot/replay audit catalog review assignments", () => {
  it("assigns, lists, inspects, clears, and reapplies reviewed presets through the shared query seam", async () => {
    const inlineSavedView = createSavedView(
      "saved_view_inline",
      "worker_inline_assignment",
      "2026-03-30T18:00:00.000Z",
    );
    const queuedSavedView = createSavedView(
      "saved_view_queued",
      "worker_queued_assignment",
    ...
    await catalogQuery.publishCatalogEntry({
      ...
      timestamp: "2026-03-30T18:00:05.000Z",
    });
    await catalogQuery.publishCatalogEntry({
      id: "catalog_queued",
      savedViewId: queuedSavedView.id,
      timestamp: "2026-03-30T18:00:06.000Z",
    });
    await visibilityQuery.setCatalogVisibilityState(
      "catalog_inline",
      ownerViewer,
      "personal",
      "2026-03-30T18:00:07.000Z",
    );
    await visibilityQuery.setCatalogVisibilityState(
      "catalog_queued",
      ownerViewer,
      "shared",
      "2026-03-30T18:00:08.000Z",
    );
    await reviewSignalQuery.setCatalogReviewSignal(
      "catalog_inline",
      ownerViewer,
      {
        note: "Inline preset verified for owner follow-up",
        state: "reviewed",
      },
      "2026-03-30T18:00:09.000Z",
    );
    await reviewSignalQuery.setCatalogReviewSignal(
      "catalog_queued",
      peerViewer,
      {
        note: "Queued preset ready for handoff",
        state: "recommended",
      },
      "2026-03-30T18:00:10.000Z",
    );
    ...
    await assignmentQuery.setCatalogReviewAssignment(
      ...
      ownerViewer,
      {
        ...
        handoffNote: "Owner keeps the inline preset",
      },
      "2026-03-30T18:00:11.000Z",
    );
    await assignmentQuery.setCatalogReviewAssignment(
      "catalog_queued",
      ownerViewer,
      ...
      "2026-03-30T18:00:12.000Z",
    );

    const ownerAssignments =
      await assignmentQuery.listAssignedCatalogEntries(ownerViewer);
    const peerAssignments =
      await assignmentQuery.listAssignedCatalogEntries(peerViewer);
    const otherScopeAssignments =
      await assignmentQuery.listAssignedCatalogEntries(otherScopeViewer);
    const inspectedAssignment =
      await assignmentQuery.getCatalogReviewAssignment(
        "catalog_queued",
        ownerViewer,
      );
    const appliedAssignment = await assignmentQuery.applyCatalogEntry(
      "catalog_queued",
      peerViewer,
    );
    const clearedAssignment =
      await assignmentQuery.clearCatalogReviewAssignment(
        "catalog_queued",
        ownerViewer,
      );
    ...
    expect(otherScopeAssignments.totalCount).toBe(0);
    ...
    expect(
      appliedAssignment?.application.application.application.navigation
        .drilldowns[0]?.result.runId,
    ).toBe("run_worker_queued_assignment");
